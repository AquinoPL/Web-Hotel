...
<head>
    ...
    <title>Ranking de Bebidas del Bar</title>
    ...
    <div class="container">
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if ($mes && $anio) {
                ...
                $query = $conexion->prepare("CALL RankingBebidasBar(?, ?)");
                $query->bind_param("ii", $mes, $anio);
                $query->execute();
                $resultado = $query->get_result();

                echo "<h1><i class='fas fa-glass-martini-alt'></i> Ranking de Bebidas del Bar</h1>";
                echo "<div class='report-result'>";
                echo "<table>";
                echo "<thead><tr><th>Bebida</th><th>Cantidad Vendida</th></tr></thead>";
                echo "<tbody>";

                while ($fila = $resultado->fetch_assoc()) {
                    echo "<tr><td>{$fila['product_name']}</td><td>{$fila['TotalCantidad']}</td></tr>";
                }
        ...
        ?>
            <h1><i class="fas fa-glass-martini-alt"></i> Ranking de Bebidas del Bar</h1>
