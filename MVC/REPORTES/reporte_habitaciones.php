...
<head>
    ...
    <title>Ranking de Habitaciones Ocupadas</title>
    ...
    <div class="container">
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if ($fecha_inicio && $fecha_fin) {
                ...
                $query = $conexion->prepare("CALL RankingHabitacionesOcupadas(?, ?)");
                $query->bind_param("ss", $fecha_inicio, $fecha_fin);
                $query->execute();
                $resultado = $query->get_result();

                echo "<h1><i class='fas fa-bed'></i> Ranking de Habitaciones Ocupadas</h1>";
                ...
                while ($fila = $resultado->fetch_assoc()) {
                    echo "<tr>
                            <td>{$fila['room_id']}</td>
                            <td>{$fila['TotalReservas']}</td>
                            <td>{$fila['DiasOcupados']}</td>
                          </tr>";
                }
        ...
        ?>
            <h1><i class="fas fa-bed"></i> Ranking de Habitaciones Ocupadas</h1>
