    ...
    <div class="container">
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if ($fecha_inicio && $fecha_fin) {
                ...
                $query = $conexion->prepare("CALL ListadoHuespedesActuales(?, ?)");
                $query->bind_param("ss", $fecha_inicio, $fecha_fin);
                $query->execute();
                $resultado = $query->get_result();

                echo "<h1><i class='fas fa-users'></i> Listado de Huéspedes</h1>";
                ...
                while ($fila = $resultado->fetch_assoc()) {
                    echo "<tr>
                            <td>{$fila['huesped_id']}</td>
                            <td>{$fila['huesped_name']}</td>
                            <td>{$fila['huesped_lastname']}</td>
                            <td>{$fila['huesped_typedoc']}</td>
                            <td>{$fila['huesped_doc']}</td>
                            <td>{$fila['user_phonenumber']}</td>
        ...
        ?>
            <h1><i class="fas fa-users"></i> Listado de Huéspedes</h1>
