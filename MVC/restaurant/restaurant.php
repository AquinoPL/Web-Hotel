...
<head>
    ...
    <title>Restaurant - Hotel Cielo</title>
...
<body>
    ...
    if (isset($_POST['addProduct'])) {
        $selectedProduct = [
            ...
            'quantity' => $_POST['food_quantity'] // Añadir cantidad
        ];
    ...
    function insertConsumo($conn, $reservationId, $productId, $quantity) {
        ...
        $stmtConsumo = $conn->prepare("CALL RegistrarConsumoComedor(?, ?, ?)");
    ...
    if (isset($_POST['addResult'])) {
        ...
        if (isset($_SESSION['selectedProducts']) && count($_SESSION['selectedProducts']) > 0) {
            ...
            foreach ($_SESSION['selectedProducts'] as $product) {
                ...
                $stmtProduct = $conn->prepare("CALL BuscarMenuPorNombre(?)");
                ...
                if ($row = $result->fetch_assoc()) {
                    $productId = $row['food_id']; // Obtener el food_id del resultado
    ...
    <main>
        ...
        <section id="productCatalog" class="room-catalog">
            ...
            <div class="room-grid">
                <?php
                $sql = "SELECT p.food_id, p.food_name, p.food_description, 
                            p.food_price, p.food_type, p.food_image
                        FROM Menu_Comedor p";
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        $imagePath = !empty($row['food_image']) ? $row['food_image'] : 'https://via.placeholder.com/300x200';
                        echo "<div class='room-card'>
                                <div class='room-info'>
                                    <img src='{$imagePath}' alt='Imagen del producto'>
                                    <h3>{$row['food_name']}</h3>
                                    <p>{$row['food_description']}</p>
                                    <p>Precio: \${$row['food_price']}</p>
                                    <form method='POST' action='' style='display: flex; align-items: center; gap: 5px;'>
                                        ...
                                        <button type='button' onclick='updateQuantity(this, -1)' style='padding: 5px; background-color: #f0f0f0; border: 1px solid #ccc; cursor: pointer;'>-</button>
                                        <input type='number' name='food_quantity' value='1' min='1' max='100' readonly style='width: 60px; text-align: center;'>
                                        <button type='button' onclick='updateQuantity(this, 1)' style='padding: 5px; background-color: #f0f0f0; border: 1px solid #ccc; cursor: pointer;'>+</button>

                                        <button type='submit' name='addProduct' class='btn btn-info'>Agregar al Menu</button>
                                    </form>
                                </div>
                            </div>";
                    }
        ...
        <script>
            ...
            function updateQuantity(button, change) {
                var input = button.parentElement.querySelector('input[name="food_quantity"]');
